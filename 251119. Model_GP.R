...
# ===================== LOAD & CLEAN =====================
df_raw <- read_excel("250827. Inputs.xlsx", sheet = "GP_full") %>%
  filter(For_R == 1) %>%
...
# require (and check) v_mean_i_int, sd_i, v_threshold_i, p_subgroupi, c_offset_i.
outcome_nums <- df_raw %>%
...
count_below_threshold <- function(n_total, p_subgroup, threshold, mean_arm, sd_common, uptake = 1) {
  n_sub <- n_total * p_subgroup
  p_imp <- pnorm(threshold, mean = mean_arm, sd = sd_common)
  ceiling(n_sub * p_imp * uptake)
}
...
run_cca <- function(params, uptake = 1, burnout = 0, complaint = 0, format = c("f2f","online")) {
  ...
  with(as.list(params), {
    ...
    for (i in outcome_nums) {
      ...
      p_sub <- params[[paste0("p_subgroup", i)]]
      ...
      n_int <- count_below_threshold(n_tot, p_sub, thr, m_int, sd_i, uptake)
      n_ctl <- count_below_threshold(n_tot, p_sub, thr, m_ctl, sd_i, 1)
    ...
    # --- satisfaction (incremental vs control) ---
    ...
    visit_add  <- n_pt_compo1 * uptake * c_care_component1_per_unit * n_care_component1_units
    gross_cost <- (train_cost + opp_cost + visit_add)
    
    # --- provider TIME (HOURS) ---
    training_hours <- n_providers * cfg$hours * cfg$sessions
    visit_hours    <- (n_patients_eligible * uptake * n_care_component1_units) / 60
...
# ============== RUN ==============
# Base case:
df_result_base <- run_cca(df_input_base, uptake = 1, burnout = 0, complaint = 0)
...
# Scenario 2: empathy care extends the consultation duration by 10 minutes instead of 2
df_inputs_scenario2 <- df_input_base
df_inputs_scenario2$n_care_component1_units <- 10
df_result_scenario2 <- run_cca(df_inputs_scenario2, uptake = 1, burnout = 0)
 

# Scenario 3: burnout = 1 and complaint = 1
...
# Run DSA (use the same format as the base-case)
...
tornado_f2f_gp <- plot_tornado_topn(
  df_dsa_results,
  df_result_base,
  format = "f2f",
...
# export to PNG
#ggsave("tornado_f2f_gp.png", plot = tornado_f2f_gp, width = 8, height = 6, dpi = 300)
    ...
    for (i in seq_len(nrow(psa_info))) {
      ...
      if (!is.na(dis) && dis != "") {
        ...
        val <- switch(dis,
                      ...
                      "gamma"    = rgamma(1, shape = p1, scale = p2), # if scale, not rate
...
# Run PSA for your chosen format
...
violin_plot_psa_gp <- ggplot(df_result_psa, aes(x = "", y = cost_per_patient)) +
...
sen_plots <- grid.arrange(
  tornado_f2f_gp, nullGrob(), violin_plot_psa_gp,
  ncol = 3, widths = c(2.5, 0.2, 1))

# Export to PNG
#ggsave("violin_plot_psa_gp.png", plot = violin_plot_psa_gp, width = 8, height = 6, dpi = 300)
ggsave("sen_plots_gp.png", plot = sen_plots, width = 15, height = 6, dpi = 300)
...
# Run the export (adjust the filename per case)
export_case_study(
  res_list      = res_list,
  outcome_nums  = outcome_nums,
  out_file      = "251119. CCA_results_GP_case.xlsx",   
  tornado_plot  = tornado_f2f_gp,                          
  violin_plot   = violin_plot_psa_gp,                   
...
label_results <- function(df) {
  lookup <- c(
    "Strategy" = "strategy",
    "Offset cost on pain" = "cost_offset_outcome1",
    "Offset cost on anxiety" = "cost_offset_outcome2",
    "Offset cost on depression" = "cost_offset_outcome3",
...
# create a df_cost_component to store all cost component, turn offsetcost to negative and relabel 
df_cost_component <- bind_rows(res_list, .id = "Analysis") %>%
  select(Analysis, train_cost, opp_cost, visit_add, starts_with("cost_offset_outcome"), burn_offset, complaint_offset) %>%
  label_results() %>%
